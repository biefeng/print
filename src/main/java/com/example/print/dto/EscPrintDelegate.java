...
public class EscPrintDelegate {
    public void print(BasicPrintData printData,TemplateProp prop) {
        try {
            ...
            if (PrinterType.THERMAL.getValue().equals(printerType)) {
                printer = new EscThermalPrinter(ip, printMode);
            } else if (PrinterType.THERMAL.getValue().equals(printerType)) {
            ...
            for (ElementProp unit : prop.getElements()) {
                ...
                switch (type) {
                    case "text":
                        printer.text(value, Font.A_SPECIAL, unit.getFontWidth(), unit.getFontHeight(), hl, hh, vl, vh);
                        break;
                    case "barCode":
                        ...
                        printer.barCode(value, barType, barHeight, barWidth, pos, hl, hh, vl, vh);
                        break;
                    default:
                        break;
                }
